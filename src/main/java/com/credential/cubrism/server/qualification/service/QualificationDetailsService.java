...
public class QualificationDetailsService {
    ...
    private void saveQualificationDetails(QualificationDetailsApiGetDTO qualificationDetailsApiGetDTO) {
        qualificationDetailsRepository.deleteByCode(qualificationDetailsApiGetDTO.getCode()); // 데이터가 중복되지 않도록 기존 데이터 삭제
        QualificationDetails qualificationDetails = new QualificationDetails();
        qualificationDetails.setCode(qualificationDetailsApiGetDTO.getCode());
        qualificationDetails.setTendency(qualificationDetailsApiGetDTO.getTendency());
        qualificationDetails.setAcquisition(qualificationDetailsApiGetDTO.getAcquisition());
        setExamSchedules(qualificationDetailsApiGetDTO, qualificationDetails);
        setExamFees(qualificationDetailsApiGetDTO, qualificationDetails);
        setExamStandards(qualificationDetailsApiGetDTO, qualificationDetails);
        setPublicQuestions(qualificationDetailsApiGetDTO, qualificationDetails);
        setRecommendBooks(qualificationDetailsApiGetDTO, qualificationDetails);
        qualificationDetailsRepository.save(qualificationDetails);
    }

    private void setExamSchedules(QualificationDetailsApiGetDTO qualificationDetailsApiGetDTO, QualificationDetails qualificationDetails) {
        List<ExamSchedules> examSchedulesList = Optional.ofNullable(qualificationDetailsApiGetDTO.getSchedule()).orElse(Collections.emptyList()).stream()
                .map(schedule -> {
